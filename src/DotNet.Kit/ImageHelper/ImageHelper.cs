...
namespace DotNet.Kit
{
    public class ImageHelper
    {
        ...
        /// 下载图片到本地
        /// </summary>
        /// <param name="pSourceUrl">图片地址</param>
        /// <param name="pPath">保存路径</param>
        public static void DownloadPic(string pSourceUrl, string pPath)
        {
            var request = WebRequest.Create(pSourceUrl);
            var response = request.GetResponse();
            Stream reader = response.GetResponseStream();
            FileStream writer = new FileStream(pPath, FileMode.OpenOrCreate, FileAccess.Write);
            try
            {
